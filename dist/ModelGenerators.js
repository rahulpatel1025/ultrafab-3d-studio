 ...
 */
export function createRoundedBoxGeometry(width, height, depth, radius = 0.012, segments = 4) {
...
export class ModelGenerators {
    materials;
    assets;
    constructor(materialLibrary, assetLoader) {
     ...
     */
    createModularLegFrame(width = 0.6, height = 0.86, depth = 0.65) {
        ...
        for (const x of xOffsets) {
            for (const z of zOffsets) {
                ...
                leg.castShadow = true;
                group.add(leg);
                const foot = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.025, 0.035, 16), this.materials.stainlessMat);
     ...
     */
    createModularCabinet(width = 0.6, height = 0.82, depth = 0.62, type = "drawer-door") {
        const group = new THREE.Group();
        group.userData = {
     ...
     */
    createCornerCabinet(width = 0.9, height = 0.86, depth = 0.65) {
        const group = new THREE.Group();
        group.userData = {
        ...
        const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.006, 0.18, 12), this.materials.stainlessMat);
     ...
     */
    createElectricalRaceway(length = 2.0, height = 0.14, depth = 0.08, numSockets = 3) {
        const group = new THREE.Group();
        group.userData = {
        ...
        for (let i = 1; i <= numSockets; i++) {
            ...
            for (const pinY of [-0.015, 0.015]) {
                const pin = new THREE.Mesh(new THREE.CylinderGeometry(0.007, 0.007, 0.003, 12), new THREE.MeshBasicMaterial({ color: 0x1e293b }));
                pin.rotation.x = Math.PI / 2;
                pin.position.set(-0.018, pinY, 0.005);
                plate.add(pin);
            }
            const earthPin = new THREE.Mesh(new THREE.CylinderGeometry(0.009, 0.009, 0.003, 12), new THREE.MeshBasicMaterial({ color: 0x1e293b }));
            earthPin.rotation.x = Math.PI / 2;
            earthPin.position.set(-0.018, 0.025, 0.005);
            plate.add(earthPin);
            const rocker = new THREE.Mesh(new THREE.BoxGeometry(0.016, 0.028, 0.006), this.materials.socketRockerMat);
     ...
     */
    createOverheadGlassCabinetSuite(numUnits = 6, unitWidth = 0.45, height = 0.65, depth = 0.35) {
        const group = new THREE.Group();
     ...
     */
    createDualSinkStation(width = 1.5, height = 0.9, depth = 0.75) {
        const group = new THREE.Group();
        group.userData = {
        ...
        for (let s = 0; s < sinkOffsets.length; s++) {
            ...
            group.add(sinkBasin);
            const drain = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.01, 16), this.materials.stainlessMat);
            ...
            const spoutCurve = new THREE.Mesh(new THREE.TorusGeometry(0.06, 0.012, 12, 24, Math.PI), this.materials.stainlessMat);
            spoutCurve.rotation.z = Math.PI / 2;
            spoutCurve.position.set(-0.04, 0.28, 0);
            faucetGroup.add(spoutCurve);
            const valveKnob = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.018, 0.025, 12), this.materials.waterValveMat);
     ...
     */
    createReagentShelvingRack(length = 2.2, height = 0.75, depth = 0.35) {
        const group = new THREE.Group();
        group.userData = {
        ...
        for (const shelfY of [0.42, 0.72]) {
            ...
            for (const railZ of [-depth / 2 + 0.015, depth / 2 - 0.015]) {
                const rail = new THREE.Mesh(new THREE.CylinderGeometry(0.005, 0.005, length * 0.94, 12), this.materials.stainlessMat);
     ...
     */
    createThermalEquipment(type = "heraeusOven", width = 0.7, height = 0.8, depth = 0.65) {
        const group = new THREE.Group();
        const titles = {
        ...
        const body = new THREE.Mesh(bodyGeo, type === "muffleFurnace" ? this.materials.ovenCharcoalMat : this.materials.ovenBodyMat);
        body.position.set(0, height / 2, 0);
        body.castShadow = true;
        ...
        group.add(panel);
        const screen = new THREE.Mesh(new THREE.PlaneGeometry(0.18, 0.065), new THREE.MeshBasicMaterial({ color: type === "muffleFurnace" ? 0xef4444 : 0x06b6d4 }));
        screen.position.set(-width * 0.22, 0, 0.012);
        panel.add(screen);
        for (let d = 0; d < 2; d++) {
            const dial = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.016, 0.015, 16), this.materials.stainlessMat);
    ...
    buildLShapedWorkstationScene() {
        ...
        const wallLower = new THREE.Mesh(new THREE.BoxGeometry(5.8, winBottomY, wallThick), this.materials.roomWallMat);
        ...
        const wallUpper = new THREE.Mesh(new THREE.BoxGeometry(5.8, wallUpperHeight, wallThick), this.materials.roomWallMat);
        ...
        const wallSideL = new THREE.Mesh(new THREE.BoxGeometry(leftSideWidth, winHeight, wallThick), this.materials.roomWallMat);
        ...
        const wallSideR = new THREE.Mesh(new THREE.BoxGeometry(rightSideWidth, winHeight, wallThick), this.materials.roomWallMat);
        ...
        const sill = new THREE.Mesh(createRoundedBoxGeometry(winWidth + 0.08, 0.04, 0.22, 0.006, 2), this.materials.mainMat);
        sill.position.set(0, -winHeight / 2, 0.04);
        winGroup.add(sill);
        // Window Outer Frame
        const frameOuter = new THREE.Mesh(createRoundedBoxGeometry(winWidth, winHeight, 0.08, 0.006, 2), this.materials.mainMat);
        winGroup.add(frameOuter);
        // Vertical Central Mullion
        const vertMullion = new THREE.Mesh(new THREE.BoxGeometry(0.024, winHeight - 0.04, 0.03), this.materials.mainMat);
        vertMullion.position.set(0, 0, 0);
        winGroup.add(vertMullion);
        // Horizontal Transom Bar
        const horizTransom = new THREE.Mesh(new THREE.BoxGeometry(winWidth - 0.04, 0.024, 0.03), this.materials.mainMat);
        ...
        const glassPane = new THREE.Mesh(new THREE.BoxGeometry(winWidth - 0.04, winHeight - 0.04, 0.008), glassMat);
        ...
        const drwHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.006, diagW * 0.45, 12), this.materials.stainlessMat);
        ...
        const doorHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.006, 0.16, 12), this.materials.stainlessMat);
        doorHandle.position.set(diagW * 0.32, 0.12, 0.018);
        doorMesh.add(doorHandle);
        const lockMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.008, 0.01, 12), this.materials.stainlessMat);
    ...
    buildOvenRoomScene() {
        ...
        for (let c = -1.5; c <= 1.5; c += 1.0) {
            ...
            const seat = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.05, 16), this.materials.grayAccentMat);
        ...
        root.add(ductGroup);
        const ductMain = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, roomD * 0.85, 24), this.materials.stainlessMat);
        ductMain.rotation.x = Math.PI / 2;
        ductMain.position.set(0, 0, 0);
        ductGroup.add(ductMain);
        for (const dropX of [-0.6, 0.6]) {
            const dropFlue = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.9, 16), this.materials.stainlessMat);
            dropFlue.position.set(dropX, -0.45, -roomD / 2 + 0.45);
            ductGroup.add(dropFlue);
            const canopyHood = new THREE.Mesh(new THREE.ConeGeometry(0.35, 0.25, 16, 1, true), this.materials.stainlessMat);
