...
export class AssetLoader {
  ...
  private initLoaders(): void {
    ...
    this.loadingManager.onProgress = (url: string, itemsLoaded: number, itemsTotal: number) => {
      ...
      window.dispatchEvent(
        new CustomEvent("ultrafab:asset-progress", {
  ...
  public async loadModel(url: string, options: AssetLoadOptions = {}): Promise<THREE.Group> {
    ...
    if (!this.gltfLoader) {
      if (fallbackFn) return fallbackFn(this.materials);
    ...
    try {
      ...
      const model: THREE.Group = gltf.scene || gltf.scenes[0];
   ...
   */
  public async loadAll(manifest: AssetManifestItem[]): Promise<Map<string, THREE.Group>> {
