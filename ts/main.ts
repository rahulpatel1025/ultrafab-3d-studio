...
export class UltraFabStudioApp {
  ...
  public currentMode: LabModelMode = "lBench";
  public clock: THREE.Clock;

  public sceneManager!: SceneManager;
  public materialLibrary!: MaterialLibrary;
  public assetLoader!: AssetLoader;
  public modelGenerators!: ModelGenerators;
  public animationController!: AnimationController;
  public uiManager!: UIManager;
  public iframeBridge!: IframeBridge;
   ...
   */
  public async switchModel(mode: LabModelMode, isInitial: boolean = false): Promise<void> {
    ...
    if (this.animationController.isExploded) {
      ...
      if (explodeBtn) {
        ...
        if (label) label.textContent = "Dynamic Teardown";
      }
    }
...
if (typeof window !== "undefined") {
  const bootstrap = () => {
    if (!(window as any).__ULTRAFAB_APP__) {
      try {
        (window as any).__ULTRAFAB_APP__ = new UltraFabStudioApp();
      } catch (err) {
        ...
        if (overlay) {
          overlay.innerHTML = `
            <div class="p-6 text-center text-rose-400 font-sans max-w-sm">
              ...
              <div class="text-xs text-slate-300 mb-4">${(err as Error)?.message || "WebGL initialization notice"}</div>
