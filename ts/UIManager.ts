...
import { LabModelMode, ComponentMetadata, MetricHUDData } from "./types.js";
...
export class UIManager {
  ...
  public pointerDownX: number = 0;
  public pointerDownY: number = 0;
  ...
  private handleClick(event: PointerEvent): void {
    ...
    const intersects = this.raycaster.intersectObjects(
      this.app.sceneManager.currentModelRoot ? [this.app.sceneManager.currentModelRoot] : [],
      true
    );

    if (intersects.length > 0) {
      let target: THREE.Object3D | null = intersects[0].object;
      let meta: ComponentMetadata | null = null;

      while (target && target !== this.app.sceneManager.scene) {
        if (target.userData && target.userData.title) {
          meta = target.userData as ComponentMetadata;
          break;
        }
  ...
  public showInspectionPopover(meta: ComponentMetadata): void {
    ...
    if (catEl) catEl.textContent = meta.category || "Laboratory Specification";
    if (titleEl) titleEl.textContent = meta.title || "UltraFab Component";
    if (specsEl) specsEl.textContent = meta.specs || "SEFA-8 Compliant Construction";
    if (descEl) descEl.textContent = meta.description || "Precision engineered laboratory component.";
    if (oemEl) oemEl.textContent = meta.oemCode || "UF-MOD-2026";

    popover.classList.remove("hidden");
    popover.classList.add("flex");
  }

  public hideInspectionPopover(): void {
    const popover = document.getElementById("inspectionPopover");
    if (popover) {
   ...
   */
  public setSkeletonLoading(active: boolean): void {
  ...
  public updateMetricsHUD(modelMode: LabModelMode): void {
    const metrics: Record<LabModelMode, MetricHUDData> = {
      lBench: {
    ...
    if (lenEl) lenEl.textContent = data.linearLength;
    if (areaEl) areaEl.textContent = data.counterArea;
    if (sktEl) sktEl.textContent = data.sktCount;
    if (compEl) compEl.textContent = data.compliance;
  }
  ...
  public bindEvents(): void {
    ...
    modelButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        const mode = btn.getAttribute("data-model-mode") as LabModelMode;
    ...
    cameraButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        const preset = btn.getAttribute("data-camera-preset") as any;
    ...
    if (explodeBtn) {
      explodeBtn.addEventListener("click", () => {
        ...
        if (label) label.textContent = nextState ? "Assemble 3D" : "Dynamic Teardown";
      });
    }
    ...
    if (wireframeBtn) {
      wireframeBtn.addEventListener("click", () => {
        ...
        if (label) label.textContent = nextState ? "Solid Render" : "3D Wireframe";
      });
    }
